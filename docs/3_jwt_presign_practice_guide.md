...
웹/인증 서버 구동 — **`operation/` 스크립트 사용 권장**:
```bash
cd auth-server && npm install   # 최초 1회 의존성 설치 후 프로젝트 루트로 돌아옴
cd ..

bash operation/startup.sh       # 백그라운드 기동 · PID 기록 · 로그 시작
tail -f operation/.run/auth-server.log   # 기동 확인 (Ctrl+C 로 빠져나와도 서버는 계속 실행)
```
```bash
bash operation/stop.sh          # 서버 중지 (멱등 — 이미 중지돼 있어도 안전)
```

> `startup.sh`는 `config/env.sh` + `.state/resources.env`에서 `API_ENDPOINT`를 자동 로드합니다.
> 이미 기동 중이면 중복 실행 없이 안내 후 종료합니다.
> 런타임 파일(`operation/.run/`)은 `.gitignore` 처리되어 리포에 올라가지 않습니다.

<details>
<summary>수동 실행이 필요한 경우 (참고)</summary>

```bash
# auth-server/ 디렉토리 안에서 실행해야 함 (package.json 위치)
source .state/resources.env
cd auth-server
API_ENDPOINT="$API_ENDPOINT" npm start          # 포그라운드 (터미널 점유)
# 또는
nohup env API_ENDPOINT="$API_ENDPOINT" npm start > /tmp/auth-server.log 2>&1 &
```
> ⚠️ 프로젝트 루트에서 `npm start`하면 `ENOENT: package.json not found` 오류가 납니다.
</details>
...
### H-0. auth-server 중지
```bash
bash operation/stop.sh          # PID 파일로 프로세스 종료 · 이미 중지 상태여도 안전
```
