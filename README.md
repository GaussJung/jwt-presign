...
```
> Windows에서 실행한다면 `scripts\powershell\00_prereqs.ps1` … 처럼 .ps1 버전을 쓴다.

## 보안 (교육 한정)
- `jwt_private_key.pem` 은 **공개 리포 금지**. 폐쇄 커뮤니티 배포, 수업 후 폐기.
- EC2 IAM 관리자 권한은 샌드박스 전용. 실습 후 `99_teardown` + EC2 종료.

자세한 실습 절차: `docs/3_jwt_presign_practice_guide.md`

**전체 문서:** `1_lab-guide` · `2_개념설명(PDF)` · `3_실습가이드` · `4_구조도` · `5_운영매뉴얼` — 모두 `docs/` 아래.
