...
let token = null;     // 로그인 후 받은 JWT (브라우저 메모리에만 보관)
...
// 페이지 로드 시 API 주소를 서버 설정에서 읽어온다(2단계).
(async () => {
  try {
...
$("loginBtn").onclick = async () => {
  try {
    ...
    token = data.token;
    $("who").textContent = `로그인: ${data.sub}`;
    $("uploadBtn").disabled = false; $("refreshBtn").disabled = false;
